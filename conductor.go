...
func apiget(ticket string) {
	result := execute("-c", "curl", "--request", "GET", "--url", jira.URL+"search/jql?jql=summary%20~%20"+ticket+"&fields=key&maxResults=1", "--header", "Authorization: Basic "+token.Jira, "--header", "Accept: application/json")
